...
"""
Usage:
    train_model_crossseizure.py --model=<p> --db=<d> --overlap=<o>
...
from utils.models import Net_Wang_1d, Net_Wang_2d, early_stop_wang
    ...
    for patient in patients:
        ...
        for (training_files, testing_files) in iter(patient):
            ...
            if OPTS["--model"] == "wang_1d":
                model = Net_Wang_1d((num_channels, x_train.shape[2]))
            elif OPTS["--model"] == "wang_2d":
                model = Net_Wang_2d((num_channels, x_train.shape[2], 1))
                x_train = x_train[..., np.newaxis]

            model.fit(x_train, y_train, epochs=MLCONF["epochs"],
                      verbose=1, validation_split=0.2,
                      batch_size=100, callbacks=[early_stop_wang(DBCONF["patience"])])
